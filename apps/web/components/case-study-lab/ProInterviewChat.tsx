...
import type { ProCaseStudySlots } from "@repo/prompts";
import { ConsoleInput, ConsoleButton } from "@/components/console";

type Msg = { role: "assistant" | "user"; content: string };

export default function ProInterviewChat({
  id,
  firstReply,
  initialReady,
  onReady,
  ...
  onReady: (slots: ProCaseStudySlots) => void;
}) {
  ...
  async function send(e: React.FormEvent) {
    ...
    setInput("");
    try {
      const res = await fetch("/api/case-study-lab-pro/turn", {
        method: "POST",
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {(ready || slots) && (
        <ConsoleButton type="button" onClick={() => slots && onReady(slots)} className="self-start" disabled={!slots}>
