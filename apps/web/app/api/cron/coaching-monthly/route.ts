...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Generating monthly coaching reports for ${monthName}`);
    ...
    for (const user of users || []) {
      try {
        ...
        // Skip if no calls (need at least a few for monthly)
    ...
    return NextResponse.json({
      success: true,
      period: { start: periodStart, end: periodEnd, month: monthName },
      results,
    });
  } catch (error) {
    console.error('Monthly coaching cron error:', error);
