...
function WelcomeContent() {
  ...
  useEffect(() => {
    async function loadUser() {
      ...
      if (!user) {
        router.push('/login?returnTo=/visibility-lab-pro/welcome');
        return;
      }
  ...
  const handleContinue = async () => {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) {
        router.push('/visibility-lab-pro');
        return;
      }
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <div className="max-w-2xl mx-auto text-center">
        ...
        <h1 className="font-anton text-[clamp(36px,6vw,56px)] text-white mb-4 tracking-wide">
          WELCOME TO{' '}
          <span className="text-[#FFDE59]">VISIBILITY LAB PRO</span>
        </h1>

        <p className="text-xl text-[#CCCCCC] mb-8 leading-relaxed">
          You just upgraded your visibility intelligence.
          <br />
          Time to see exactly how your brand shows up — and where it doesn&apos;t.
        </p>
        ...
        <div className="bg-[#1a1a1a] border border-[#333] p-8 mb-8 text-left">
          ...
          <p className="text-[#B3B3B3] mb-6 font-poppins">
            We&apos;ll use this to personalize your visibility audits:
          </p>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#666] text-sm font-poppins">
                Example: Brand strategy consulting for B2B SaaS companies looking to differentiate...
              </p>
            </div>
          </div>
        </div>
        ...
        >
          {saving ? 'SAVING...' : '[ RUN YOUR FIRST VISIBILITY AUDIT ]'}
        </button>
